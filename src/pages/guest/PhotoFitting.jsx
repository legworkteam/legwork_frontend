...
export default function PhotoFitting() {
  ...
  return (
    <div className="mx-auto flex min-h-svh max-w-[430px] flex-col bg-ink text-bg">
      <Header transparent />
      ...
      <div className="flex flex-1 flex-col items-center justify-center gap-6 px-6 py-8 text-center">
        <div>
          ...
          <p className="mt-2 text-sm text-bg/70">사진을 올리면 AI가 제품을 합성해드려요</p>
        </div>

        <button
          type="button"
          onClick={() => fileInputRef.current?.click()}
          className="flex h-72 w-full items-center justify-center overflow-hidden rounded-2xl border-2 border-dashed border-gold/60 bg-bg/5"
        >
          {previewUrl ? (
            <img src={previewUrl} alt="업로드한 사진 미리보기" className="h-full w-full object-cover" />
          ) : (
            <span className="flex flex-col items-center gap-2 text-bg/60">
